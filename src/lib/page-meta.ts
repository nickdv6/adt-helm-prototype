...
export const PAGES: PageMeta[] = [
  ...
  { path: '/api/dispatch-to-rip', title: 'Dispatcher API · /api/dispatch-to-rip', summary: 'Helm-side upstream handoff. POST {pr_id} → validates + generates external_job_name + picks hot folder + builds XML + returns the four-write transaction plan.',
    category: 'Meta', wave: 'Wave 1', dataSource: 'live', dataNote: 'Live endpoint. Returns generated XML + writes; production runs writes in a transaction.', sourcePath: 'app/api/dispatch-to-rip/route.ts' },
  ...
  { path: '/batch-ticket',  title: 'Batch Ticket · Heat Press',   summary: 'Group printed PRs into a single heat-press run.',
    category: 'Production', blueprintId: 'S30', wave: 'Wave 1', dataSource: 'mock', dataNote: 'Batch grouping is UI-only — no heat_press_batches table; batches vanish on reload.', sourcePath: 'app/batch-ticket/page.tsx' },
  ...
  { path: '/fulfillment-requests', title: 'Fulfillment Requests', summary: 'All FPRs with filter + status.',
    category: 'Fulfillment', wave: 'Wave 1', dataSource: 'mixed', dataNote: 'Reads FPR seed data, but the Shopify-stored-inventory source mode is inline mock.', sourcePath: 'app/fulfillment-requests/page.tsx' },
